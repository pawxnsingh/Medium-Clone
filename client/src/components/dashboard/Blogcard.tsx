...
const Blogcard = ({ item }: any) => {
  ...
  return (
    <div className="flex flex-col border-b font-notosans mt-7 pr-2">
      <Link to={`/${username}`} className="flex py-2 items-center gap-2 mb-1">
        ...
        />
        <h1 className="text-xs font-medium hover:underline">{name}</h1>
      </Link>
      <div className="flex justify-between items-start ">
        ...
        <div className="ml-8 md:h-[7rem] w-[10rem]">
          {articleImage[0] ? (
            <img
              src={articleImage[0]}
              width={160}
            ...
            <img
              src="https://images.unsplash.com/photo-1626285348736-b6588ad36018?q=80&w=1887&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"
              width={160}
              alt="blogPicture"
              className="w-full h-full object-cover"
            />
      ...
      <div className="flex justify-between items-center mt- 3 mb-5 mr-2 lg:mr-[10rem] text-[0.9rem] text-gray-600">
        <div className="flex gap-3">
          <div>{date}</div>
          <div className="flex items-center gap-1">
            <Clap width={14} height={14} />
            <h1>{clap.length}</h1>
          </div>
          <div className="flex items-center gap-1">
            <RiMessage3Fill />
...
export const Clap = ({ width, height }: { width: number; height: number }) => {
  return (
    <svg
